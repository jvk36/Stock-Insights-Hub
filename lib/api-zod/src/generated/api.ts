...
export const GetInsiderTransactionsResponse = zod.object({
  ...
  transactions: zod.array(
    zod.object({
      ...
      ownershipRelationship: zod.string().nullish(),
      holdingSharesAfter: zod.number().nullish(),
      activityPctOfHoldings: zod.number().nullish(),
      is10b51Plan: zod.boolean(),
      isCompensationRelated: zod.boolean(),
      compensationReason: zod.string().nullish(),
      contextFlags: zod.array(zod.string()),
      accessionNumber: zod.string(),
      formUrl: zod.string(),
    }),
  ),
  coverage: zod.object({
    source: zod.enum(["sec", "yahoo"]),
    availableFilings: zod.number(),
    requestedFilings: zod.number(),
    fetchedFilings: zod.number(),
